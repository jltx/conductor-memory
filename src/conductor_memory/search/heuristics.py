...
class HeuristicMetadata:
    ...
    # Simple file detection (for skipping LLM summarization)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'is_simple_file': self.is_simple_file,
...
class HeuristicExtractor:
    ...
    def extract_file_metadata(
        ...
        try:
            ...
            # Detect simple files that don't need LLM summarization
            self._detect_simple_file(metadata, content)
    ...
    # Generated file markers to detect auto-generated code
    ...
    def _detect_simple_file(self, metadata: HeuristicMetadata, content: str) -> None:
        """
        Detect if a file is "simple" and doesn't need LLM summarization.
        
        Simple files include:
        - Empty modules (≤10 lines, 0 functions, 0 classes)
        - Barrel/re-export files (__init__.py, index.ts/js)
        - Type-only files (TypeScript/Java with only interfaces/types)
        - Constants-only files (0 functions, only const assignments)
        - Generated code (has DO NOT EDIT, @generated, etc. markers)
        
        Sets metadata.is_simple_file and metadata.simple_file_reason.
        """
        file_path = metadata.file_path
        file_name = Path(file_path).name.lower()
        language = metadata.language
        
        # Count lines (excluding empty lines for more accurate assessment)
        lines = content.split('\n')
        total_lines = len(lines)
        non_empty_lines = len([l for l in lines if l.strip()])
        
        # Count definitions
        func_count = len(metadata.functions) + len(metadata.methods)
        class_count = len(metadata.classes)
        interface_count = len(metadata.interfaces)
        
        # Check for generated file markers in header (first 20 lines)
        header = '\n'.join(lines[:20]).upper()
        for marker in self.GENERATED_FILE_MARKERS:
            if marker.upper() in header:
                metadata.is_simple_file = True
                metadata.simple_file_reason = "generated_code"
                return
        
        # Python-specific rules
        if language == 'python':
            # All __init__.py files are barrel/re-export files
            if file_name == '__init__.py':
                metadata.is_simple_file = True
                metadata.simple_file_reason = "barrel_reexport"
                return
            
            # Small Python files with no functions or classes
            if total_lines <= 30 and func_count == 0 and class_count == 0:
                metadata.is_simple_file = True
                metadata.simple_file_reason = "constants_only" if non_empty_lines > 3 else "empty_module"
                return
        
        # TypeScript/JavaScript-specific rules
        elif language in ['typescript', 'javascript']:
            # index.ts/js with only exports (no functions)
            if file_name in ['index.ts', 'index.js', 'index.tsx', 'index.jsx']:
                if func_count == 0 and class_count == 0:
                    metadata.is_simple_file = True
                    metadata.simple_file_reason = "barrel_reexport"
                    return
            
            # Type-only files (only interfaces/types, no functions)
            if func_count == 0 and class_count == 0 and interface_count > 0:
                metadata.is_simple_file = True
                metadata.simple_file_reason = "type_definitions"
                return
        
        # Java-specific rules
        elif language == 'java':
            # package-info.java files
            if file_name == 'package-info.java':
                metadata.is_simple_file = True
                metadata.simple_file_reason = "barrel_reexport"
                return
            
            # Interface-only files (only interfaces, no implementations)
            if func_count == 0 and class_count == 0 and interface_count > 0:
                metadata.is_simple_file = True
                metadata.simple_file_reason = "type_definitions"
                return
        ...
        # Empty modules: very small files with no code
        if total_lines <= 10 and func_count == 0 and class_count == 0:
            metadata.is_simple_file = True
            metadata.simple_file_reason = "empty_module"
            return
        
        # Constants-only files: small files with no functions or classes
        # but with some content (likely just constants/configuration)
        if total_lines <= 30 and func_count == 0 and class_count == 0 and non_empty_lines > 3:
            # Check if it looks like a constants file (mostly assignments)
            assignment_count = sum(1 for line in lines if '=' in line and not line.strip().startswith('#'))
            if assignment_count >= non_empty_lines * 0.4:  # 40% of lines are assignments
                metadata.is_simple_file = True
                metadata.simple_file_reason = "constants_only"
                return